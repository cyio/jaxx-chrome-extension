...
(function (jaxx) {
    ...
    var ShapeShiftController = (function () {
        function ShapeShiftController(sendTransactionsController) {
            ...
            this.$view.load('js/app/shape-shift/shape-shift-view.html', function () {
         ...
         * @param {Boolean} reset
         * */
        ShapeShiftController.prototype.hideList = function (reset) {
            ...
            //reset the amount input if the shapeshift item has changed to a new value and validate
            // the send button
            if (reset) {
                this.sendTransactionsController.updateFieldWithBalance(this.sendTransactionsController.$amount, '0');
                this.sendTransactionsController.amountDidChangeListener();
            }
        };
        ...
        ShapeShiftController.prototype.changeCoinShiftToType = function (symbol) {
            ...
            //need this to check for a change
            var old = this.ctrTo;
            var el = this.$listTo.find("[data-symbol=" + symbol + "]");
            this.ctrTo = jaxx.Registry.getCryptoControllerBySymbol(symbol);
            this.$selected = el;
            this.lastSymbol = symbol;
            //if there is a change reset the amount input and evaluate the send button
            if (this.ctrTo.symbol === old.symbol) {
                this.hideList(false);
            }
            else {
                this.hideList(true);
        ...
        ShapeShiftController.prototype.shift = function (amount, callBack) {
            ...
            if (!this.ctrFrom || !this.ctrTo) {
                callBack({ error: 'From and To must be set' });
                console.error('From and To must be set - ' + "To: " + this.ctrTo + " From: " + this.ctrFrom);
                return;
            }
        ...
        ShapeShiftController.prototype.updateShapeShiftView = function () {
            ...
            if (!this.ctrFrom) {
                console.error('No current Crypto-controller found.');
                return;
            }
        ...
        ShapeShiftController.prototype.render = function () {
            ...
            this.$From.children('.cssCoinText').text(this.ctrFrom.threeLetterCode);
            this.$From.css({ "background-image": "url(" + this.ctrFrom.icon + ")" });
            this.$From.css({ "filter": "hue-rotate(" + this.ctrFrom.hueRotate + "deg)" });
            this.$To.children('.cssCoinText').text(this.ctrTo.threeLetterCode);
        ...
        ShapeShiftController.prototype.downloadExchange = function (callBack) {
            ...
            this.render();
            if (!this.ctrFrom || !this.ctrTo) {
                console.error('From and To must be set');
                return;
            }
