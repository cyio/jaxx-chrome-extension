...
(function (jaxx) {
    var TransactionsView = (function () {
        ...
        TransactionsView.prototype.populateTransactionHistory = function () {
            ...
            this.offset = 0;
            this.listHeight = 0;
            this.limit = this.incrementValue < this.txHistory.transactions.length ? this.incrementValue : this.txHistory.transactions.length;
        ...
        TransactionsView.prototype.initEvents = function () {
            ...
            var self = this;
            $('#TransactionsListContainer').scroll(function () {
                if ($('#TransactionsListContainer').scrollTop() + $('#TransactionsListContainer').height() >= self.listHeight) {
        ...
        TransactionsView.prototype.displayTransactions = function () {
            var symbol = this.txHistory.symbol;
            var ctr = jaxx.Registry.getCryptoControllerBySymbol(symbol);
            var trs = this.txHistory.transactions;
            var blockexplorer_url = this.txHistory.blockexplorer_url;
            var trsConfirmations = this.txHistory.trsConfirmations || 6;
            ...
            if (ctr.supportsTransactionHistory() == false) {
                ...
                while (this.offset < this.limit) {
                    html += TransactionsView.formatRow(trs[this.offset], tpl, symbol, trsConfirmations, blockexplorer_url);
                    this.offset++;
                }
                /**
                 ...
                 * */
                if (this.limit < trs.length - this.incrementValue) {
                    this.limit += this.incrementValue;
                }
                else {
                    this.limit = trs.length;
                }
            }
            else
             ...
             */
            this.listHeight += $('.cssTransactionHistoryRow').height() * this.incrementValue;
        };
