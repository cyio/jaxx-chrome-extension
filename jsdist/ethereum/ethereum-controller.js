    ...
    var EthereumController = (function (_super) {
        ...
        EthereumController.prototype.buildTransaction = function (amountDecimals, addressTo, isMax, customGasLimit, customData) {
            ...
            this.getNonces(addresses).done(function (nonces) {
                ...
                var chainId = (_this.config.symbol === "ETC") ? 61 : 1;
                ...
                _this.config.gasPrice, gasLimit, _this.symbol, isMax, customData, chainId);
        ...
        EthereumController.prototype.restoreHistory = function (callBack) {
            ...
            if (!jaxx.Registry.isOnline) {
                jaxx.Registry.CTR$.triggerHandler(jaxx.Registry.CTR_ERROR_NO_INTERNET_ON_RESTORE_HISTORY, this);
                if (callBack)
                    callBack(jaxx.Registry.CTR_ERROR_NO_INTERNET_ON_RESTORE_HISTORY, null);
                if (!this.isActive)
                    return;
                setTimeout(function () { return _this.restoreHistory(callBack); }, 10000);
                return;
            }
            this.coinService.restoreHistory(0).done(function (result) {
                ...
                _this.isRestoringHistory = false;
                /// when initialization failed by event  KILL_HISTORY  do not broadcast CTR_SERVICE_ERROR
                // because this event forced to stop initialization
                if (!!error.error && error.error === jaxx.Registry.KILL_HISTORY) {
                }
                else
                    jaxx.Registry.CTR$.triggerHandler(jaxx.Registry.CTR_SERVICE_ERROR, _this);
                if (callBack)
                    callBack(error, null);
                _this.onError(error);
            });
