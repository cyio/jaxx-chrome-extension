...
(function (jaxx) {
    var PaperWalletTransactionController = (function () {
        ...
        PaperWalletTransactionController.prototype.prepareSweepTransactionCoin = function (privateKey, symbol, callback) {
            ...
            coinController.coinService.downlaodUTXOs([publicAddress]).done(function (res) {
                ...
                if (Number(spendable) > 0) {
                    ...
                    var miningFeeSatoshi = coinController.getMiningFee();
                    ...
                    if (coinController.symbol === 'BTC') {
                        try {
                            transaction = jaxx.TransactionsUtilsBitcoin.buildBTC(amountSatoshi, coinController.getCurrentAddress(), utxos, privateKeys, null, //miningFeeSatoshi,
                            addressChange, coinController.config.network, coinController.symbol, true, coinController.getMiningPrice());
                        }
                        catch (e) {
                            return callback('There is an error in the application. Jaxx is not able to sweep this private key', null);
                        }
                    }
                    else if (coinController.symbol === 'BCH') {
                        try {
                            transaction = jaxx.TransactionsUtilsBitcoin.buildBCH(amountSatoshi, coinController.getCurrentAddress(), utxos, privateKeys, miningFeeSatoshi, addressChange, coinController.config.network, coinController.symbol, true, coinController.getMiningPrice());
                        }
                        catch (e) {
                            return callback('There is an error in the application. Jaxx is not able to sweep this private key', null);
                        }
                        ...
                        try {
                            transaction = jaxx.TransactionsUtilsBitcoin.build(amountSatoshi, coinController.getCurrentAddress(), utxos, privateKeys, miningFeeSatoshi, addressChange, coinController.config.network, coinController.symbol, true);
                        }
                        catch (e) {
                            return callback('There is an error in the application. Jaxx is not able to sweep this private key', null);
                        }
        ...
        * @param {Function} callback
        * */
        PaperWalletTransactionController.prototype.prepareSweepTransactionEthereum = function (privateKey, callback) {
            var privateKeyHex = new thirdparty.Buffer.Buffer(privateKey, 'hex');
            var signatures = {};
            signatures['address'] = privateKeyHex;
            var ethAddressToSweep = [this.ethereumAddressFromPrivateKey(privateKey)];
            if (ethAddressToSweep[0]) {
                var ethereumController_1 = jaxx.Registry.getCryptoControllerBySymbol('ETH');
        ...
        * @param {String} privateKey
        * @return {String}*/
        PaperWalletTransactionController.prototype.ethereumAddressFromPrivateKey = function (privateKey) {
            //Create a fake tx
            var mockUpTxRaw = {
                ...
                gasLimit: thirdparty.web3.toHex(jaxx.Registry.getCryptoControllerBySymbol('ETH').config.gasLimit),
