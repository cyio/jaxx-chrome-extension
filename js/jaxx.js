...
$(document).ready(function () {
    ...
    jaxx.Registry.API_KEY = localStorage.getItem('jaxxAPIKey');
    var appVersion = jaxx.Registry.application.appVersion;
    var apiKeyId = localStorage.getItem('jaxxAPIKeyId');
    if(!jaxx.Registry.API_KEY) {
        var name = guid();
        ...
        if(name && browserCodeName && browserName && browserVersion && browserLanguage && platform && userAgentHeader) {
          $.ajax('https://pricing.jaxx.io/dev/get-api-key?name=' + name + '&browserCodeName=' + browserCodeName +
            '&browserName=' + browserName + '&browserVersion=' + browserVersion + '&browserLanguage=' + browserLanguage +
            '&platform=' + platform + '&userAgentHeader=' + userAgentHeader + '&jaxxVersion=' + appVersion + '&apiKeyId=' + apiKeyId, {
            type: 'POST',
            dataType: 'json',
            contentType: "application/json",
          }).done(function(response) {
            if(response.user_api_key) {
              jaxx.Registry.API_KEY = response.user_api_key;
              localStorage.setItem('jaxxAPIKey', response.user_api_key);
              localStorage.setItem('jaxxAPIKeyId', response.user_key_id);
            }
          }).fail(function() {
            console.error('could not generate api key');
          }).always(function() {
      ...
      $.ajax('https://pricing.jaxx.io/dev/get-api-key?jaxxVersion=' + appVersion + '&apiKeyId=' + apiKeyId, {
        type: 'POST',
        dataType: 'json',
        contentType: "application/json",
      }).done(function(response) {
        console.log('updated jaxxVersion');
      }).fail(function() {
...
var Navigation = (function () {
    ...
    var pushSettings = function (settingsName, callback) {
        ...
        if (settingsName === 'pageSetupSecurityPinCode') {
            $('.validateMnemonic').val('');
        }
...
//            Navigation.flashBanner("Openning up desktop browser");
    ...
    var toggleCurrency = function (pCurrency, pEnabled) {
        ...
        if (pEnabled && (!$.inArray(pCurrency, _currenciesEnabled) > -1)) {
            ...
            $(".exchangeRateList").find('[value=' + pCurrency + ']').find('.cssSelectedCurrency').find('.cssCircleUnchecked').addClass('cssCurrencyChecked');
            ...
            if (activeCurrencies.length > 1) {
                ...
                $(".exchangeRateList").find('[value=' + pCurrency + ']').find('.cssSelectedCurrency').find('.cssCircleUnchecked').removeClass('cssCurrencyChecked');
                Navigation.updateCurrencyList();
            }
        }
...
function specialAction(actionName, element) {
    ...
    if (actionName === 'refresh') {
        ...
        if(ctr.isToken){
            ctr.checkBalanceCurrentReceive();

        }else{
            ctr.restoreHistory(function (error) {
...
function updateDefaultWalletList() {
    $('.settings.setDefaultWallet .setDefaultWalletList div').each(function () {
        ...
        if (elementCoinType === g_JaxxApp.getSettings().getDefaultCoinType()) {
            ...
            element.find('.cssCircleUnchecked').addClass('cssCurrencyChecked').removeClass('cssCircleUnchecked');
            ...
            element.find('.cssCurrencyChecked').removeClass('cssCurrencyChecked').addClass('cssCircleUnchecked');
        }
    });
}
