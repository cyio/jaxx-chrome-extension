...
JaxxUI.prototype.refreshSizes = function() {
    ...
    // Not perfect, because each time, even when cssNotificationFooter is not necessary and not visible,
    // we still will be trying to find it with selector $(".cssNotificationFooter")
    if($(".cssNotificationFooter").is(':visible')){
        JaxxUI.prototype.refresh_cssNotificationFooter();
    }
}
...
JaxxUI.prototype.pushCurrencyMainMenuToJaxxSettings = function(){
    ...
    for (var i = 0; i < arrCurrencyRowsInList.length; i++){
        if ($(arrCurrencyRowsInList[i]).children('.cssSelectedCurrency').children('.cssCircleUnchecked').hasClass('cssCurrencyChecked'))
        {
...
{
    $(currency_parent_row).children('td.cssSelectedCurrency').children('div.cssCircleUnchecked').addClass('cssCurrencyChecked');
...
JaxxUI.prototype.closeMainMenu = function() {
    ...
    var menu = $('.menu');
    var wallet = $('.wallet');
    var mainMenuClose = $('.mainMenuCloser');
    var noScrollSize = $('.nonScrollSize');
    var hamburger = $('.material-design-hamburger__layer');
    ...
    if (!this._mainMenuToggleLocked) {
        ...
        menu.animate({"marginLeft": ["100%", 'easeOutExpo']}, {
            duration: 700,
            complete: function () {
                self.setMainMenuToggleLocked(false);
        ...
        mainMenuClose.css('display', 'none');
        noScrollSize.fadeTo(0, 500);
        ...
        // Fade background in.
        hamburger.addClass('material-design-hamburger__icon--from-arrow');
        hamburger.removeClass('material-design-hamburger__icon--to-arrow');
  ...
  //  console.error('JaxxUI.prototype.generateSettingsCryptoCurrencyRowsWalletSetup');
   ...
   // console.log('JaxxUI.prototype.generateSettingsCryptoCurrencyRows     ', cryptoCurrencies);
    ...
    for (var i = 0; i < cryptoCurrencies.length; i++) {
        ...
        var symbol =  ctr.symbol;
        var column1 = '<td class="cssSelectedCurrency"><div data-symbol="'+ symbol +'" data-id ="'+name+'" class="cssCircleUnchecked  '+(ctr.enabled?'cssCurrencyChecked':'')+'"></div></td>';
   ...
   // this.updateCryptoCurrencyBannersInHeaderCarousel();
    ...
    $(strSelectorForTable +' .scriptAction').click(function (event) { // Add the scriptAction triggers again.
        ...
        if(toggleMenuItem.hasClass('cssCircleUnchecked')){
            ...
            var enbl = toggleMenuItem.hasClass('cssCurrencyChecked');
            //var symbol = el.data('symbol');
            //var parentElement = el.closest('.cssCoinCurrency', '.coinType');

            if (toggleMenuItem.hasClass('cssCurrencyChecked'))
            {
                if (jaxx.Registry.getWalletsEnabledSorted().length > 1)
                {
                    toggleMenuItem.removeClass('cssCurrencyChecked');
                    enbl = false;
                } else {
                    return;
                }
            } else {
                toggleMenuItem.addClass('cssCurrencyChecked');
                enbl = true;
            }
...
JaxxUI.prototype.enableCryptoCurrencyInUI = function(cryptoCurrency, dontUpdateBanners) {
    ...
    $('.mainMenuWallets .coinList .coinType' + cryptoCurrency + ' .cssSelectedCurrency .cssCircleUnchecked').addClass('cssCurrencyChecked');// Make UI Change.
...
JaxxUI.prototype.disableCryptoCurrencyInUI = function(cryptoCurrency, dontUpdateBanners) {
    this._walletWasChangedInMenu = true;
    $('.mainMenuWallets .coinList .coinType' + cryptoCurrency + ' .cssSelectedCurrency .cssCircleUnchecked').removeClass('cssCurrencyChecked');// Make UI Change.
...
JaxxUI.prototype.toggleIgnoreEtcEthSplit = function() {
    ...
    if (ignoreEtcEthSplit === true) {
        $('.etcEthSplitIgnoreToggleButtonCheckArea').addClass('cssCurrencyChecked');
        $('.etcEthSplitIgnoreToggleButtonCheckArea').css('border', 'none');
    } else {
        $('.etcEthSplitIgnoreToggleButtonCheckArea').removeClass('cssCurrencyChecked');
...
JaxxUI.prototype.refresh_cssNotificationFooter = function(){
